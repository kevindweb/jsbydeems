console.log('CosineFunc Start');
//very similar to cosineFunc
//the difference is the variation on the cosine series
var pi = Math.PI;
...
function cosineFunc(input){
	...
	for(var n=2;(input**n)/f(n)>.001;n+=2){
		var thisSeries; 
		var operatorPlus = '+';
		var operatorMinus = '-';
		...
		if(count!=0&&count==1){
			//first position needs to be negative(-)
			seriesArray.unshift('-');
	...
	cosine = 1 + eval(seriesArray.join(''));
	return cosine.toFixed(4);
};
